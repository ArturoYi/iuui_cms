<template>
  <div>
    ...
    <Collapse class="collapseComponent">
      <Card class="card" name="extra" :title="$t('Router.FormUi')">
        <div style="width: 600px">
          ...
          >
            ...
            <FormItem label="Hardcore" prop="hardcore">
              <Switched v-model="form.hardcore" trueText="Yes" falseText="No" />
            </FormItem>
            <FormItem label="Other" prop="other">
              <TextArea
                placeholder="最多只能输入10个字符"
                v-model="form.other"
              />
            </FormItem>
            <FormItem prop="readme" :wrapperCol="{ offset: 6 }">
              <CheckBox v-model="form.readme"
                >我已阅读 <a>服务条款</a>
              </CheckBox>
            </FormItem>
...
export default {
  data() {
    ...
    return {
      ...
      rules: {
        ...
        one: [{ required: true, message: "必选项" }],
        system: [{ required: true, message: "请选择系统类型" }],
        hardcore: [{ required: true, message: "必选项" }],
  ...
  methods: {
    setValue() {
      this.form = {
        email: "Cyr@tranbiot.cn",
        password: "Cyr@tranbiot.cn",
        repassword: "Cyr@tranbiot.cn",
        phone: "13888888888",
        captcha: "8888",
        gender: "1",
        slider: 5,
