...
export default {
  ...
  render() {
    let { $slots, disabled, Dropdown, opened, Menu, SubMenu, icon, rendered } =
      this;
    let key = this.$vnode.key || "sub_" + this._uid;
    const {
      currentMode,
    ...
    let types =
      currentMode == "horizontal" || inlineCollapsed ? "vertical" : currentMode;

    const preCls = Dropdown ? "dropdown-menu-submenu" : "menu-submenu";

    let titleProps = {
      class: `u-${preCls}-title`,
      on: {
    ...
    const titleNode = (
      <div {...titleProps}>
        <span class={`u-${preCls}-inner`}>
          {icon ? <Icon type={icon} /> : null}
          {isVnode(title) ? title : <span>{title}</span>}
        </span>
        <Icon
          type={
            (showInline && !inlineCollapsed) ||
            (currentMode == "horizontal" && SubMenu == null)
              ? "chevron-down"
              : "chevron-forward"
          }
          class={`u-${preCls}-arrow`}
        />
      </div>
    );
    ...
    const popupProps = {
      slot: "content",
      class: [`u-${preCls}-popup`],
      style: {
        "max-height": inlineCollapsed ? "390px" : null,
        "min-width": `${this.minWidth}px`,
        "margin-left":
          theme == "dark" && !SubMenu && mode == "horizontal" ? "-16px" : null,
      },
      on: {
        mouseenter: () => {
        ...
        rendered ? (
          <Collapse
            ...
            name={
              "u-" +
              preCls +
              (showInline && !inlineCollapsed && !Dropdown ? "-slide" : "-fade")
            }
          >
            {childNode}
          </Collapse>
    ...
    const classes = [
      `u-${preCls}`,
      {
        [`u-${preCls}-active`]: this.active,
        [`u-${preCls}-selected`]: selected && !Dropdown,
        [`u-${preCls}-opened`]: opened,
        [`u-${preCls}-disabled`]: disabled,
      },
    ];
  ...
  methods: {
    ...
    openChange() {
      if (this.Menu) {
        let { currentMode, defaultOpenKeys, accordion, inlineCollapsed } =
          this.Menu;
