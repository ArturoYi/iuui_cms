...
export default {
  ...
  methods: {
    ...
    renderCol(hasHead) {
      ...
      columns.forEach((col, i) => {
        ...
        let colProps = {
          ...
          class: [
            {
              "u-table-selection-col": hasCheckbox,
              // 'u-table-cell-fixed-left': col.fixed == 'left',
              // 'u-table-cell-fixed-right': col.fixed == 'right',
            },
          ],
        };
        if (!width) {
      ...
      if (this.hasExpand) {
        cols.unshift(<col class="u-table-expand-icon-col"></col>);
      }
    ...
    renderTH(col, left, right) {
      ...
      let props = {
        class: {
          "u-table-cell-ellipsis": col.ellipsis,
          "u-table-cell-selection": hasCheckbox,
          "u-table-cell-fixed-left": col.fixed == "left",
          "u-table-cell-fixed-right": col.fixed == "right",
          "u-table-cell-fixed-left-last": col.last,
          "u-table-cell-fixed-right-first": col.first,
          "u-table-cell-sorter": hasSort,
      ...
      let inner =
        hasCheckbox && col.checkType !== "radio" ? (
          <Checkbox
      ...
      return (
        <th {...props}>
          <span class="u-table-header-col">
            {inner ? <span class="u-table-header-title">{inner}</span> : null}
    ...
    renderHead() {
      ...
      let head = [],
        ...
        right =
          cols
            ...
            .map((c) => c.width)
            .reduce((a, b) => a + b, 0) +
          (this.height ? this.scrollBarHeight : 0);
        ...
        if (Array.isArray(col)) {
          ...
          if (i == 0 && this.width) {
            ths.push(
              <th
                rowSpan={col.length}
                class="u-table-cell-fixed-right"
                style={{ width: this.scrollBarHeight + "px" }}
              ></th>
            );
          }
          head.push(<tr>{ths}</tr>);
      ...
      if (this.height != undefined) {
        ...
        if (hasFR) {
          cls.push("u-table-cell-fixed-right");
        }
        ths.push(
          <th style={{ width: this.scrollBarHeight + "px" }} class={cls}></th>
        );
      }
      if (!head.length) {
        head.push(<tr>{ths}</tr>);
    ...
    asyncScroll(e, target) {
      ...
      this.$emit("ping", ping);
      // this.$refs.table.setAttribute('class', `u-table11-ping-${ping}`)
    },
  },
  render() {
    ...
    let rootProps = {
      ...
      //   // { [`u-table-ping-${type}`]: width != undefined },
      // ],
      style: {},
      on: {},
    };
