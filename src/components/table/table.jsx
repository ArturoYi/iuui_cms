...
let { Remove, Add } = { Remove: "remove", Add: "add" };
export default {
  name: "Table",
  props: {
    bordered: Boolean,
  ...
  render() {
    ...
    //custom head
    if (header) {
      content.push(<div class="u-table-header"> {header}</div>);
    }
    ...
    let checkAll =
      data.filter((x) => x._checked).length == data.length && data.length > 0;
    ...
    data.forEach((d, i) => {
      ...
      _columns.forEach((c, j) => {
        ...
        if (c.key || c.type) {
          ...
          let props = {
            class: [
              {
                "u-table-cell-ellipsis": c.ellipsis,
                "u-table-cell-selection": c.type == "selection",
                "u-table-cell-fixed-left": c.fixed == "left",
                "u-table-cell-fixed-left-last": c.last,
                "u-table-cell-fixed-right-first": c.first,
                "u-table-cell-fixed-right": c.fixed == "right",
      ...
      if (expandNode) {
        tr.unshift(
          <td class="u-table-row-expand-icon-cell">
            <Icon
              onClick={(e) => this.onExpand(d)}
              type={d._expanded ? Remove : Add}
            />
          </td>
        );
      }
      let trProps = {
        class: { "u-table-row-hover": d._hover },
      ...
      if (expandNode) {
        tbody.push(
          <tr v-show={d._expanded} class="u-table-expand-row">
            <td></td>
            <td colSpan={_columns.length}>{expandNode(d)}</td>
          </tr>
        );
      }
    });
    ...
    const rootProps = {
      ...
      class: [
        "u-table",
        {
          "u-table-fixed": isTableFixed,
          "u-table-sm": this.size == "small",
          "u-table-lg": this.size == "large",
          // 'u-table-fixed-header': isFixedHeader,
          "u-table-bordered": this.bordered,
          [`u-table-ping-${this.ping}`]: width !== undefined,
        },
      ],
    };
    ...
    //custom footer
    if (footer) {
      content.push(<div class="u-table-footer">{footer}</div>);
    }
    if (loading) {
      content.push(<Spin />);
    }
