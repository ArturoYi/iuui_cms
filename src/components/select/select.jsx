...
import { hasProp, getChild, isNotEmpty } from "../utils/utils";
// import { Sync, Close, CloseCircle, ChevronDown } from 'icons'
export default {
  name: "Select",
  props: {
    placeholder: String,
    ...
    getOptions() {
      ...
      if (this.filterable && queryKey) {
        ...
        childs = childs.filter((c) => {
          let label =
            c.componentOptions.propsData.label ||
            c.componentOptions.children[0].text;
  ...
  render() {
    ...
    const classes = [
      "u-select",
      {
        "u-select-disabled": disabled,
        "u-select-open": opened,
        "u-select-borderless": bordered === false,
        "u-select-lg": size == "large",
        "u-select-sm": size == "small",
        "u-select-light": theme == "light",
        "u-select-has-icon": !!icon,
        "u-select-circle": shape == "circle" && !multiple,
      },
    ];
    ...
    const queryNode = (
      <div v-show={this.showSearch} key="search" class="u-select-search-wrap">
        <input {...queryProps} />
        <span class="u-select-search-mirror" ref="mirror">
          {queryKey}
        </span>
      </div>
    );
    ...
    const props = {
      ...
      props: {
        ...
        // transfer: false,
        transfer: transfer,
        transitionName: "u-select",
        className: [
          "u-select-dropdown",
          { "u-select-dropdown-multiple": this.multiple },
        ],
      },
      on: {
        hide: () => {
    ...
    const placeNode =
      placeholder && (!label || !label.length) && !queryKey ? (
        <div class="u-select-placeholder">{placeholder}</div>
      ) : null;
    const tags = multiple
      ? label.map((c, i) => {
          return (
            <span class="u-select-tag" key={c.key}>
              {c.label}
    ...
    const labelStyle = {
      opacity: this.showSearch ? 0.4 : 1,
      display: queryKey.length ? "none" : "",
    };
    const labelsNode = multiple ? (
      <transition-group tag="div" class="u-select-labels" name="u-select-tag">
        {tags}
        {queryNode}
      </transition-group>
    ) : (
      <div class="u-select-label" style={labelStyle}>
        {label}
      </div>
    );
    let isSearch = "search" in this.$listeners;
    ...
    !isSearch &&
      showArrow &&
      childNode.push(<Icon class="u-select-arrow" type={arrowIcon} />);
    ...
    const selectCls = [
      "u-select-selection",
      {
        "u-select-has-clear": showClear,
      },
    ];
    showClear &&
      childNode.push(
        <Icon class="u-select-clearable" type="close-circle" onClick={clear} />
      );

    return (
      <div tabIndex="0" class={classes} style={styles}>
        <div class={selectCls} onClick={toggleDrop} ref="rel">
          {icon ? <Icon type={icon} class="u-select-icon" /> : null}
          {childNode}
        </div>
        {overlay}
      </div>
