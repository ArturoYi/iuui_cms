...
let { Sync, RemoveCircleOutline, AddCircleOutline, ChevronUp } = {
  Sync: "sync",
...
export default {
  ...
  methods: {
      ...
      !disabled &&
        children.forEach((item) => {
          if (!item.disabled) {
            ...
            checked
              ? index < 0 && defaultCheckedKeys.push(key)
              : index > -1 && defaultCheckedKeys.splice(index, 1);
    ...
    updateParentCheck({ defaultData, VNode }) {
      ...
      let checkedLength = normal.filter(
        ({ key }) => defaultCheckedKeys.indexOf(key) > -1
      ).length;
  ...
  render(h) {
    ...
    if ((hasChilds || "load-data" in Tree.$listeners) && isLeaf !== true) {
      let arrowCls = ["u-tree-arrow", { "u-tree-arrow-open": expand }];
      let arrowNode = (
        <span class={arrowCls} onClick={this.handleExpand}>
          <Icon
            type={
              loading
                ? Sync
      ...
      itemNode.push(<span class="u-tree-commes"></span>);
    }
    if (checkable) {
      let props = {
        ...
        on: {
          change:
            this
              .handleCheck /*  input: e => { this.$set(this.data, 'checked', e) } */,
        },
      };
    ...
    let iconNode = null;
    if (icon && showIcon) {
      iconNode = <Icon type={icon} class="u-tree-icon" />;
    }

    let titleCls = ["u-tree-title", { "u-tree-title-selected": selected }];
    ...
    let onProps = getTranstionProp("u-tree-slide");
    const itemProps = {
      attrs: {},
      class: [
        "u-tree-item",
        {
          "u-tree-item-disabled": disabled,
          "u-tree-item-drag": this.draged,
          "u-tree-item-drop": this.droped && !disabled,
          "u-tree-item-extra-hidden": !showExtra,
          "u-tree-item-selected": directory && selected,
        },
      ],
      on: {},
    };
    ...
    if (Tree.$scopedSlots.extra) {
      ...
      extraNode = <span class="u-tree-item-extra">{extra}</span>;
    }
    return (
      <div class="u-tree-children">
        <div {...itemProps}>
          {itemNode}
          {extraNode}
        </div>
        <transition-group class="u-tree-item-children" tag="div" {...onProps}>
          {childs}
        </transition-group>
      </div>
    );
