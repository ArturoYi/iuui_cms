...
import { getChild } from "../utils/utils.js";
  ...
  methods: {
    ...
    setParentHalf(data = [], key) {
      for (let i = 0; i < data.length; i++) {
        ...
        if (
          keys.indexOf(key) >= 0 &&
          this.defaultCheckedKeys.indexOf(item.key) < 0
        ) {
          //符合要求
    ...
    onDrop(event, { node }) {
      ...
      if (
        ...
        if (!node.children || !node.children.length) {
          node.children = [];
          this.defaultExpandedKeys.indexOf(node.key) < 0 &&
    ...
    return (
      <div
        class={[
          "u-tree",
          {
            "u-tree-show-line": showLine,
            "u-tree-directory": directory,
          },
        ]}
      >
        {this.renderChild()}
