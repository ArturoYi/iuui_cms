...
export default {
  ...
  render() {
    ...
    dateIcon && childNode.push(<Icon type={dateIcon} class="u-icon-calendar" />)
    if (isRange) {
      ...
      let p1 = placeholder[0] || t('datePicker.startDate'), p2 = placeholder[1] || t('datePicker.endDate')
      let v1 = label[0], v2 = label[1]
      if (v1) {
        childNode.push(<div class="u-datepicker-value">{v1}</div>)
      } else {
        childNode.push(<div class="u-datepicker-placeholder">{p1}</div>)
      }
      childNode.push(<div class="u-datepicker-separator">~</div>)
      if (v2) {
        childNode.push(<div class="u-datepicker-value">{v2}</div>)
      } else {
        childNode.push(<div class="u-datepicker-placeholder">{p2}</div>)
      }
    } else {
      placeholder = placeholder || t('datePicker.placeholder')
      if (label) {
        childNode.push(<div class="u-datepicker-value">{label}</div>)
      } else if (placeholder) {
        childNode.push(<div class="u-datepicker-placeholder">{placeholder}</div>)
      }
    }
    ...
    const props = {
      props: {
        className: ['u-datepicker-dropdown', { 'u-datepicker-range-dropdown': isRange }],
        transfer: transfer,
        selection: this.$el,
        value: this.opened,
        placement: 'bottom-left',
        transitionName: 'u-date-picker'
      },
      on: {
        input: e => {
          this.opened = e
    ...
    showClear && childNode.push(<Icon class="u-datepicker-clearable" type='close-circle' onClick={this.clear} />)
    const selectCls = [
      "u-datepicker-selection", {
        "u-datepicker-has-clear": showClear
      }
    ]
    const classes = ['u-datepicker',
      { 'u-datepicker-open': opened },
      { 'u-datepicker-range': isRange },
      { 'u-datepicker-borderless': bordered === false },
      { 'u-datepicker-sm': size == 'small' },
      { 'u-datepicker-lg': size == 'large' },
      { 'u-datepicker-disabled': disabled },
      { 'u-datepicker-light': theme == 'light' },
      { 'u-datepicker-circle': shape == 'circle' },
    ]
    return (
