...
let { CheckmarkCircle, CloseCircle, Close, Checkmark } = {
  CheckmarkCircle: "checkmaru-circle",
...
export default {
  ...
  methods: {
    renderTip() {
      ...
      return <div class="u-progress-text">{text}</div>;
    },
    renderCircle(percent, strokeColor, dashboard) {
      ...
      let d = `M 50,50 
        ...
        style = {
          strokeDasharray: `${
            (percent / 100) * (len - (dashboard ? gapDegree : 0))
          }px ${len}px`, //长度,间距 ,实线和虚线的长度
    ...
    renderGress() {
      ...
      if (type == "line") {
        ...
        return (
          <div class="u-progress-inner">
            <div class="u-progress-bg" style={sty}></div>
          </div>
        );
      } else if (type == "circle") {
  ...
  render() {
    ...
    let classes = [
      ...
      `u-progress-${type}`,
      `u-progress-${status}`,
      { "u-progress-sm": type == "line" && size == "small" },
      { "u-progress-hide-info": !this.showInfo },
    ];
