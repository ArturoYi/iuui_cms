  ...
  const instance = new Vue({
    ...
    render(h) {
      ...
      let icons = {
        ...
        error: "close-circle",
        success: "checkmaru-circle",
        warning: "alert-circle",
        confirm: 'help-circle'
      }
      //header 
      let header = h('div', { attrs: { class: 'u-toast-header' } }, [
        (type || icon) ? h(Icon, { class: 'u-toast-icon', props: { type: (icons[type] || icon), color } }) : null,
        h('div', { attrs: { class: 'u-toast-title' } }, title)
      ])

      //body
      let body = h('div', { attrs: { class: 'u-toast-content' }, domProps: { innerHTML: content } })
      //footer
      ...
      if (type == 'confirm') {
        footerNode.unshift(h(Button, { on: { click: this.cancel } }, cancelText || t('modal.cancel')))
      }
      let footer = h('div', { attrs: { class: 'u-toast-footer' } }, footerNode)

      let contentNode = h('template', { slot: 'content' }, [header, body, footer])
      let classes = 'u-modal u-toast ' + (icons[type] ? 'u-toast-' + type : '')
      return h(Modal, {
        attrs: { class: classes },
        props: {
          value: this.visible,
