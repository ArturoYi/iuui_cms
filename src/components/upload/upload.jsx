...
let { DocumentTextOutline, Close, AlertCircle, Add } = {
...
function getUuid() {
  return `u-upload-${timestamp}-${count++}`;
}
export default {
  name: "Upload",
  props: {
    ...
    selectFiles(e) {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (
          ...
          error = true;
          item.errorText = t("upload.errorFileSize");
          item.status = "error";
        }
        // if (multiple) {
  ...
  render() {
    ...
    const props = {
      class: [
        "u-upload",
        {
          ["u-upload-disabled"]: this.disabled,
          ["u-upload-picture"]: isPicture,
          ["u-upload-drag"]: draggable,
        },
      ],
    };
    // let list
    ...
    let showSelector =
      (isPicture && limit && limit > defaultFileList.length) ||
      !isPicture ||
      !limit;
    const selector = showSelector ? (
      <div class="u-upload-select">
        <div class="u-upload-add" {...addProps}>
          <input
            type="file"
            class="u-upload-file"
          ...
          {isPicture || (draggable && uploadText) ? (
            <span class="u-upload-text">{uploadText}</span>
          ) : null}
          {draggable && uploadSubText ? (
            <span class="u-upload-sub-text">
              {this.dragOver ? "松手开始上传" : uploadSubText}
            </span>
          ) : null}
        </div>
      </div>
    ...
    const filsList = () => {
      return (showUploadList && !isPicture) || isPicture ? (
        <div class={`u-upload-${isPicture ? "picture" : "file"}-list`}>
          {defaultFileList.map((item, i) => {
            let statusText =
              item.status == "success"
                ? t("upload.successful")
                : item.errorText || t("upload.failed");
            delete item.errorText;
            item.uid = item.uid || getUuid();
            return (
              <div
                class={[
                  `u-upload-file-${type}-item`,
                  `u-upload-file-status-${item.status}`,
                ]}
                key={item.uid}
              >
                <div
                  class={`u-upload-${isPicture ? "picture" : "file"}-preview`}
                >
                  {this.getPreview(item) || <Icon type={DocumentTextOutline} />}
                </div>
                <div class="u-upload-file-item-info">
                  {!isPicture ? (
                    <div class="u-upload-file-main">
                      <span class="u-upload-file-name">{item.filename}</span>
                      <span class="u-upload-file-size">{item.size}</span>
                    </div>
                  ) : null}
                  {item.status != "wait" ? (
                    <div class="u-upload-file-status">
                      {item.status == "uploading" ? (
                        ...
                        <div class="u-upload-file-status-text">
