...
import { getChild, isVnode } from "../utils/utils";
export default {
  name: "FormItem",
  props: {
    label: String,
  ...
  render() {
    ...
    const classes = [
      "u-form-item",
      {
        "u-form-item-required": required,
        "u-form-item-error": !valid,
      },
    ];
    ...
    return (
      <Row class={classes} type="flex">
        {label ? (
          <Col class="u-form-item-label" {...labelProp}>
        ...
        <Col {...wrapperProp}>
          <div class="u-form-item-content">
            {childs.map((child) => {
              if (isVnode(child)) {
                const tag = child.componentOptions
                  ? child.componentOptions.tag
                  : child.tag;
                const value = prop ? this.Form.testProp(prop) : "";
                const size =
                  (child.componentOptions &&
                    child.componentOptions.propsData.size) ||
                  this.Form.size;
                const theme =
                  (child.componentOptions &&
                    child.componentOptions.propsData.theme) ||
                  this.Form.theme;
                const shape =
                  (child.componentOptions &&
                    child.componentOptions.propsData.shape) ||
                  this.Form.shape;
                const props = {
                  props: { id, size },
                  on: {},
                ...
                if (prop) {
                  ...
                  props.on = {
                    input: (value) => {
                      if (tag) {
                        // if (['Select', 'u-select'].indexOf(tag) > -1) {
                        //   value = value
                        // }
                        // if (['Radio', 'u-radio', 'Checkbox', 'u-checkbox'].indexOf(tag) > -1) {
                        //   value = value.target.checked
                        // }
                        // if (['Input', 'u-input', 'TextArea', 'u-textarea'].indexOf(tag) > -1) {
                        //   value = value
                        // }
                ...
                if (
                  ["Input", "u-input", "TextArea", "u-textarea"].indexOf(tag) >
                  -1
                ) {
                  props.on.blur = (value) => {
                ...
                if (["FormItem", "u-form-item"].indexOf(tag) > -1) {
                  props.on.collect = (e) => {
                    this.$emit("collect", e);
                  };
                }
            ...
            <transition name="u-form-item-fade">
              {!valid ? (
                <div class="u-form-item-error-tip">{this.message}</div>
              ) : null}
            </transition>
          </div>
        </Col>
