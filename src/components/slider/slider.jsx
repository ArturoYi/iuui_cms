...
export default {
  ...
  methods: {
    ...
    getValue() {
      let { value = 0, range, min, max } = this,
        v;
      let diff = max - min;
      if (!range) {
        v = value;
    ...
    click(e) {
      let { disabled, range, vertical, step, max, min, defaultValue, reverse } =
        this;
      if (disabled) return;
    ...
    renderMark() {
      ...
      return (
        <div div class="u-slider-marks">
          {mks.map((v) => {
            const { active, sty } = this.isActice(v);
            return (
              <div
                class={[
                  "u-slider-maru-symbol",
                  { "u-slider-maru-symbol-active": active },
                ]}
                style={sty}
              />
            );
          ...
          {mks.map((v, i) => {
            ...
            return (
              <div
                class={[
                  "u-slider-maru-text",
                  { "u-slider-maru-text-active": active },
                ]}
                style={sty}
              >
                {txt[i]}
    ...
    renderTrack() {
      ...
      return (included && marks) || !marks ? (
        <div class="u-slider-track" style={{ ...trackSty }}></div>
      ) : null;
    },
    thumbProps() {
      let {
  ...
  render() {
    ...
    return (
      <div
        class={[
          "u-slider",
          { "u-slider-disabled": disabled, "u-slider-vertical": vertical },
        ]}
      >
        <div class="u-slider-bar">
          <div class="u-slider-rail" ref="rail" onClick={this.click}></div>
          {trackNode}
