...
import { getChild, measureScrollBar } from "../utils/utils";
...
export default {
  ...
  methods: {
    ...
    touchstart(e) {
      if (
        e.touches.length == 1 &&
        this.$refs.imgRef &&
        this.$refs.imgRef.contains(e.target)
      ) {
      }
    },
    mousedown(e) {
    ...
    resetPosition() {
      ...
      let panelWidth =
        $refs.panelRef && this.isShowPanel ? $refs.panelRef.offsetWidth : 0;
        ...
        return (
          <div
            class={[
              ...
              { "u-image-preview-panel-hidden": !this.isShowPanel },
            ]}
            ref="panelRef"
          >
            <span
              class="u-image-preview-panel-action"
  ...
  render(h) {
    ...
    return (
      <div class="u-image-preview-root" v-transfer={transfer}>
        <transition name="u-image-zoom">
          <div class="u-image-preview" v-show={visible}>
            <div class="u-image-preview-mask" onClick={this.close}></div>
            <div
              class="u-image-preview-wrap"
              style={{ right: panelRight + "px" }}
            >
              <ul class="u-image-preview-control">
                <li class="u-image-preview-action" onClick={this.close}>
                  <Icon type={Close} />
                </li>
                <li class="u-image-preview-action-divider" />
                {tools.map((tool) => {
                  return <li class="u-image-preview-action">{tool}</li>;
                })}
                <li class="u-image-preview-action" onClick={this.download}>
                  <Icon type={ArrowDown} />
                </li>
                <li
                  class={[
                    "u-image-preview-action",
                    { "u-image-preview-action-disabled": scale >= 5 },
                  ]}
                ...
                <li
                  class={[
                    "u-image-preview-action",
                    { "u-image-preview-action-disabled": scale <= 1 },
                  ]}
                ...
                <li
                  class="u-image-preview-action u-image-preview-action-rotate-right"
                  onClick={() => this.setRotate(0)}
                >
                  <Icon type={Refresh} />
                </li>
                <li
                  class="u-image-preview-action u-image-preview-action-rotate-left"
                  onClick={() => this.setRotate(1)}
                >
                  <Icon type={Refresh} />
                </li>
              </ul>
              <div class="u-image-preview-img-wrap" style={moveStyle}>
                {type == "media" ? (
                  <video controls {...imgPorps} />
                ) : !this.error ? (
                  <img {...imgPorps} />
                ) : (
                  <div class="u-image-preview-img-error">
                    <Icon type={IconImage} />
                  </div>
                )}
              </div>
              {showSwitch
                ...
                : null}
              {loading ? (
                <div class="u-image-preview-loading">
                  <Icon type={Sync} spin />
                </div>
              ) : null}
            </div>
